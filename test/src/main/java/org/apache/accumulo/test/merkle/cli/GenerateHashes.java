...
public class GenerateHashes {
  ...
  public Collection<Range> getRanges(Connector conn, String tableName, String splitsFile) throws TableNotFoundException, AccumuloSecurityException, AccumuloException, FileNotFoundException {
    if (null == splitsFile) {
      ...
      String line;
      java.util.Scanner file = new java.util.Scanner(new File(splitsFile), Constants.UTF8.name());
  ...
  public void run(GenerateHashesOpts opts) throws TableNotFoundException, AccumuloSecurityException, AccumuloException, NoSuchAlgorithmException,
    ...
    Collection<Range> ranges = getRanges(opts.getConnector(), opts.tableName, opts.getSplitsFile());

    run(opts.getConnector(), opts.tableName, opts.getOutputTableName(), opts.getHashName(), opts.getNumThreads(), opts.isIteratorPushdown(), ranges);
  }
